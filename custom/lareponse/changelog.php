...
dol_fiche_head($head, 'changelog', $langs->trans('Lareponse'), 0, 'lareponse_black@lareponse');

// Define path to file CHANGELOG.md.
// First check CHANGELOG-la_LA.md then CHANGELOG.md
$filefound = false;
$pathoffile = dol_buildpath('/lareponse/ChangeLog'.$langs->defaultlang.'.md', 0);
if (dol_is_file($pathoffile)) {
	$filefound = true;
}
if (! $filefound) {
	$pathoffile = dol_buildpath('/lareponse/CHANGELOG.md', 0);
	if (dol_is_file($pathoffile)) {
		$filefound = true;
	}
}
...
print load_fiche_titre($langs->trans('NewsTitle'), '', 'title_setup');
