...
/**
 * \file    test/functional/LareponseFunctionalTest.php
 * \ingroup lareponse
 ...
 *
 * @package Testlareponse
 */
class LareponseFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modLareponse')->click();
		} else {
			// Disable the module
			$this->byHref('modLareponse')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/lareponse/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('lareponse/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/lareponse/admin/about.php');
		$this->authenticate();
		return $this->assertContains('lareponse/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('lareponsewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modLareponse_LareponseTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modLareponse_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
